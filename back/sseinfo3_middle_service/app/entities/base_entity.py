...
from sqlalchemy import BOOLEAN, DATETIME, INTEGER, Column, true, INT
...
class BaseEntityDeleted(db.Model):
    __abstract__ = True

    id = Column(INTEGER, primary_key=True, autoincrement=True)
    is_deleted = Column(BOOLEAN, server_default=true(), nullable=False)
    create_time = Column(DATETIME, default=datetime.now, comment="修改时间")
    last_update_time = Column(DATETIME, default=datetime.now, onupdate=datetime.now, comment="最近修改时间", index=True)
...
class BaseEntityStatus(db.Model):
    __abstract__ = True
